...
PROTOCOL_SOURCES=$(wildcard src/protocol/*.proto)
...
build/go/models: $(VOCHAIN_SOURCES) $(VOCDONI_NODE_SOURCES) $(IPFSSYNC_SOURCES) $(PROTOCOL_SOURCES)
	rm -rf $@
	mkdir -p $@
	for f in $^ ; do \
...
build/dart: $(CLIENT_STORE_SOURCES) $(METADATA_SOURCES) $(PROTOCOL_SOURCES)
	mkdir -p $@
	for f in $^ ; do \
...
build/ts: $(CLIENT_STORE_SOURCES) $(METADATA_SOURCES) $(PROTOCOL_SOURCES)
	mkdir -p $@
	for f in $^ ; do \
